#!/usr/bin/env python3
"""
build_corpus_senato.py
----------------------
Converts Senato della Repubblica PDF transcripts to plain text and extracts
session metadata (legislature, session number, date).

Processes transcripts for 2018-2025, organized by year folder.
PDFs are sourced from the official Senato della Repubblica website.

Input:  data/raw/senato/<year>/ (PDF transcripts organized by year)
Output: data/raw/senato_txt/<year>/ (plain text)
        data/senato_metadata_2018_2025.csv (session metadata)
...
BASE     = Path(__file__).parent.parent / "data" / "raw" / "senato"
TXT_BASE = Path(__file__).parent.parent / "data" / "raw" / "senato_txt"
OUT_META = Path(__file__).parent.parent / "data" / "senato_metadata_2018_2025.csv"
...
def extract_legislature(one_line: str):
    ...
    if not m:
        return None
    mapping = {"XVII": 17, "XVIII": 18, "XIX": 19, "XX": 20}
    return mapping.get(m.group(1).upper())
...
for year in [str(y) for y in range(2018, 2026)]:
    ...
    for pdf_path in sorted(year_dir.glob("*.pdf")):
        ...
        all_rows.append({
            "year_folder": year,
            "legislature": extract_legislature(one_line),
...
print(f"Missing date_iso:    {sum(1 for r in all_rows if not r['date_iso'])}")
print(f"Missing legislature: {sum(1 for r in all_rows if not r['legislature'])}")
